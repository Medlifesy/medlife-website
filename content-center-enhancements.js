(()=>{
  ...
  async function skip(id,button){
    ...
    try{
      ...
      button.textContent='✓ لا يحتاج تصميم';
      button.classList.add('saved');
      const row=button.closest('tr');
      if(row){row.style.opacity='.55';setTimeout(()=>row.remove(),350)}
  ...
  function decorate(){
    ...
    rows.querySelectorAll('tr').forEach(row=>{
      ...
      const open=action.querySelector('button');if(!open)return;
      const match=(open.getAttribute('onclick')||'').match(/openWork\(['\"]([^'\"]+)['\"]\)/);
      const id=match&&match[1];if(!id)return;
      const b=document.createElement('button');
      b.type='button';b.className='btn light no-design-btn';b.textContent='لا يحتاج تصميم';
      b.style.marginInlineStart='6px';b.style.borderColor='#e88916';b.style.color='#9b6500';
      b.addEventListener('click',()=>skip(id,b));
      action.appendChild(b);row.dataset.noDesignReady='1';
    });
  }
  async function init(){
    me=await getMe();
    if(!me||me.type!=='design')return;
    const style=document.createElement('style');style.textContent='.no-design-btn.saved{background:#edf8f3!important;color:#176f52!important;border-color:#bfe9d8!important}.no-design-btn{white-space:nowrap}';document.head.appendChild(style);
    const rows=$('#rows');if(rows)new MutationObserver(decorate).observe(rows,{childList:true,subtree:true});
    decorate();
  }
