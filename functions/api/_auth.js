...
export async function verifyPassword(password,stored){const parts=String(stored||'').split('$');if(parts.length!==5||parts[0]!=='pbkdf2'||parts[1]!=='sha256')return false;const iterations=Number(parts[2]);if(!Number.isInteger(iterations)||iterations<10000)return false;const key=await crypto.subtle.importKey('raw',enc.encode(password),{name:'PBKDF2'},false,['deriveBits']);const bits=await crypto.subtle.deriveBits({name:'PBKDF2',salt:hexToBytes(parts[3]),iterations,hash:'SHA-256'},key,256);const actual=new Uint8Array(bits),expected=hexToBytes(parts[4]);return actual.length===expected.length&&crypto.subtle.timingSafeEqual(actual,expected)}
export async function hashToken(token){return bytesToHex(await crypto.subtle.digest('SHA-256',enc.encode(token)))}
export function randomToken(){const bytes=crypto.getRandomValues(new Uint8Array(32));return bytesToHex(bytes)}
export async function ensureAuthTables(db){await db.prepare(`CREATE TABLE IF NOT EXISTS member_sessions (id INTEGER PRIMARY KEY AUTOINCREMENT,member_id INTEGER NOT NULL,token_hash TEXT NOT NULL UNIQUE,expires_at TEXT NOT NULL,created_at TEXT NOT NULL DEFAULT CURRENT_TIMESTAMP)`).run();const r=await db.prepare('PRAGMA table_info(members)').all();const have=new Set((r.results||[]).map(x=>x.name));const cols={account_email:'TEXT',password_hash:'TEXT',account_status:'TEXT',member_code:'TEXT'};for(const [name,type] of Object.entries(cols))if(!have.has(name))await db.prepare(`ALTER TABLE members ADD COLUMN ${name} ${type}`).run()}
