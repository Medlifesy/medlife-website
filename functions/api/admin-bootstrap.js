...
export async function onRequest({request,env}){
 ...
 try{
  ...
  if(!Number.isInteger(memberId)||memberId<1)return json({success:false,error:'رقم العضوية مطلوب ويجب أن يكون رقمًا صحيحًا.'},400);
  ...
  const existing=await db.prepare('SELECT id FROM member_accounts WHERE member_id=? OR lower(username)=lower(?) LIMIT 1').bind(memberId,ADMIN_USERNAME).first();

  if(existing){
   await db.prepare(`UPDATE member_accounts SET member_id=?,username=?,password_hash=?,password_salt=?,role='admin',account_status='active',updated_at=CURRENT_TIMESTAMP WHERE id=?`).bind(memberId,ADMIN_USERNAME,passwordData.hash,passwordData.salt,existing.id).run();
  }else{
   await db.prepare(`INSERT INTO member_accounts(member_id,username,password_hash,password_salt,role,account_status,created_at,updated_at) VALUES(?,?,?,?, 'admin','active',CURRENT_TIMESTAMP,CURRENT_TIMESTAMP)`).bind(memberId,ADMIN_USERNAME,passwordData.hash,passwordData.salt).run();
  }
  return json({success:true,message:'تم إنشاء/تحديث حساب إدارة المقالات بنجاح.',member_id:memberId,username:ADMIN_USERNAME});
 }catch(error){
  console.error('admin-bootstrap error:',error);
  return json({success:false,error:`تعذر إنشاء حساب الإدارة: ${error?.message||'Unknown error'}`},500);
 }
}
