...
const ROLE_DESCRIPTIONS = {
  system_admin: 'إدارة النظام بالكامل والحسابات والأدوار والصلاحيات.',
...
function systemAdmin(me) {
  ...
  const adminRole = String(me?.admin_role || '').toLowerCase();
  return r.includes('admin') || r.includes('administrator') || r.includes('إدارة') || r.includes('مدير') || adminRole === 'system_admin' || me?.org_role === 'medical_director';
}
...
export async function onRequest({ request, env }) {
  ...
  try {
    ...
    if (request.method === 'GET') {
      const members = await db.prepare('SELECT id,full_name,email,member_code,status,account_email,account_status FROM members ORDER BY full_name').all();
    ...
    if (Number.isInteger(memberId) && memberId > 0) {
      ...
      const duplicate = await db.prepare(`SELECT id FROM members WHERE lower(COALESCE(email,''))=? OR lower(COALESCE(account_email,''))=? LIMIT 1`).bind(email, email).first();
      ...
      const passwordHashForMember = await hashPassword(password);
      await db.prepare(`INSERT INTO members (id,full_name,email,status,account_email,password_hash,account_status,member_code,medlife_role,created_at,updated_at) VALUES (?, ?, ?, 'active', ?, ?, 'active', ?, 'member', CURRENT_TIMESTAMP, CURRENT_TIMESTAMP)`).bind(newId, fullName, email, email, passwordHashForMember, generatedCode).run();
      member = { id: newId, full_name: fullName, email, account_email: email, member_code: generatedCode };
    }

    const passwordHash = await hashPassword(password);
    const parts = String(passwordHash || '').split('$');
    const salt = parts.length === 5 && parts[0] === 'pbkdf2' && parts[1] === 'sha256' ? parts[3] : null;
    const username = await createUsername(db, email, member.id);

    await db.prepare(`INSERT INTO member_accounts (member_id,username,password_hash,password_salt,role,account_status,created_at,updated_at) VALUES (?,?,?,?,?,'active',CURRENT_TIMESTAMP,CURRENT_TIMESTAMP)`).bind(member.id, username, passwordHash, salt, 'admin').run();
    ...
    if (role === 'system_admin') {
      await db.prepare("DELETE FROM medlife_admin_user_roles WHERE member_id=? AND role_key<> 'system_admin'").bind(member.id).run();
    }

    await db.prepare(`UPDATE members SET account_email=?,password_hash=?,account_status='active',updated_at=CURRENT_TIMESTAMP WHERE id=?`).bind(email, passwordHash, member.id).run();
    ...
    await db.prepare('INSERT INTO medlife_admin_audit_log(actor_member_id,action,target_type,target_id,details) VALUES(?,?,?,?,?)').bind(me.member_id,'إنشاء حساب إداري','member',String(member.id),`${ROLES[role]} — ${email}`).run();

    return json({ success: true, message: 'تمت إضافة العضو وإنشاء الحساب الإداري بنجاح.', member: { id: member.id, name: member.full_name, email }, account: { username, email, role: ROLES[role] } });
  } catch (e) {
    console.error(e);
