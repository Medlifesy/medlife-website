...
export async function onRequest({ request, env }) {
  ...
  try {
    if (request.method === 'POST' && action === 'login') {
      return await loginAdmin(request, db);
    }

    if (request.method === 'POST' && action === 'logout') {
      return await logoutAdmin(request, db);
    }

    if (request.method === 'GET' && action === 'me') {
      const admin = await authenticateAdmin(request, db);
      if (!admin) {
        return new Response(JSON.stringify({ success: false, authenticated: false }), {
          status: 401,
          headers: { 'Content-Type': 'application/json; charset=UTF-8', 'Cache-Control': 'no-store' }
        });
      }
      ...
      return new Response(JSON.stringify({
        ...
        admin: {
          ...
          email: admin.account_email || admin.email || null,
          medlife_role: admin.medlife_role || null,
          org_role: admin.org_role || null
        }
      }), {
        status: 200,
        headers: { 'Content-Type': 'application/json; charset=UTF-8', 'Cache-Control': 'no-store' }
    ...
    return new Response(JSON.stringify({ success: false, error: 'Method or action not allowed.' }), {
      status: 405,
      headers: { 'Content-Type': 'application/json; charset=UTF-8', 'Cache-Control': 'no-store' }
    });
  } catch (error) {
