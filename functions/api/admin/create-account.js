...
export async function onRequest({request,env}){
  ...
  try{
    ...
    const conflict=await db.prepare('SELECT id FROM members WHERE id<>? AND (lower(COALESCE(account_email,\'\'))=? OR lower(COALESCE(email,\'\'))=?) LIMIT 1').bind(memberId,email,email).first();
    if(conflict) return json({success:false,error:'هذا البريد مستخدم بالفعل من عضو آخر.'},409);
    ...
    if(existing){
      await db.prepare('UPDATE member_accounts SET username=?,password_hash=?,role=?,account_status=?,updated_at=CURRENT_TIMESTAMP WHERE id=?').bind(username,passwordHash,role,accountStatus,existing.id).run();
    }else{
      await db.prepare('INSERT INTO member_accounts(member_id,username,password_hash,role,account_status) VALUES(?,?,?,?,?)').bind(memberId,username,passwordHash,role,accountStatus).run();
    }

    const isGeneralAdmin=['admin','administrator','medical_director','general_team_supervisor','advisor'].includes(role);
    if(isGeneralAdmin){
      const roleLabel=role==='admin'||role==='administrator'?'admin':role;
      await db.prepare('UPDATE members SET account_email=?,password_hash=?,account_status=?,medlife_role=?,updated_at=CURRENT_TIMESTAMP WHERE id=?').bind(email,passwordHash,accountStatus,roleLabel,memberId).run();
    }else{
    ...
    return json({success:true,message:`تم إنشاء حساب ${member.full_name} بنجاح.`,member_id:memberId,username,role,account_status:accountStatus});
  }catch(error){
    console.error('admin create account error:',error);
    return json({success:false,error:'تعذر إنشاء الحساب الإداري حالياً.'},500);
  }
}
