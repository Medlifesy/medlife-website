...
export async function onRequest({request,env}){
  ...
  try{
    ...
    const members=await env.MEMBERS_DB.prepare(`SELECT id,member_code,full_name,email,account_email,phone,governorate,medlife_role,cell,status,account_status,created_at FROM members ORDER BY id DESC`).all();
    const applications=await env.MEMBERS_DB.prepare(`SELECT id,full_name,email,phone,governorate,academic_status,interest,status,created_at FROM new_member_applications ORDER BY id DESC`).all();
