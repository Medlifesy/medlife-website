...
async function updateArticle(request,db,user){
  ...
  if((status==='published'||status==='rejected')&&!['admin','reviewer'].includes(user.role))return json({success:false,error:'هذه العملية تتطلب صلاحية مدير أو مراجع.'},403);
  const article=await db.prepare('SELECT * FROM articles WHERE id=? LIMIT 1').bind(id).first(); if(!article)return json({success:false,error:'المقال غير موجود.'},404);
