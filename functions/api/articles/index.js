...
async function listPublishedArticles(db) {
    const result = await db.prepare(`
        SELECT id,title_ar,title_en,excerpt_ar,excerpt_en,author_member_id,author_name,author_email,category,image_url,status,rejection_reason,published_at,created_at,updated_at
...
async function listAllArticles(db) {
    const result = await db.prepare(`
        SELECT id,title_ar,title_en,content_ar,content_en,excerpt_ar,excerpt_en,author_member_id,author_name,author_email,category,image_url,status,rejection_reason,published_at,created_at,updated_at
        FROM articles
        ORDER BY CASE status WHEN 'pending' THEN 0 WHEN 'draft' THEN 1 WHEN 'published' THEN 2 WHEN 'rejected' THEN 3 ELSE 4 END, datetime(created_at) DESC
    `).all();
...
async function createArticle(request, articlesDb, membersDb) {
    ...
    const titleAr=clean(body.title_ar,500), titleEn=clean(body.title_en,500);
    const contentAr=clean(body.content_ar,100000), contentEn=clean(body.content_en,100000);
    const excerptAr=clean(body.excerpt_ar,2000), excerptEn=clean(body.excerpt_en,2000);
    const authorName=clean(body.author_name,200), authorEmail=clean(body.author_email,200);
    const category=clean(body.category,100), imageUrl=clean(body.image_url,2000);
    ...
    if (Number.isInteger(memberId) && memberId>0) {
        const member=await membersDb.prepare(`SELECT id,full_name,email,status FROM members WHERE id=? LIMIT 1`).bind(memberId).first();
        if (!member) return json({success:false,error:"عضو MedLife غير موجود."},400);
        authorMemberId=member.id;
    }

    const result=await articlesDb.prepare(`
        INSERT INTO articles(title_ar,title_en,content_ar,content_en,excerpt_ar,excerpt_en,author_member_id,author_name,author_email,category,image_url,status,created_at,updated_at)
...
async function updateArticle(request, db) {
    const body=await request.json();
    const id=Number(body.id), status=clean(body.status,30), rejectionReason=clean(body.rejection_reason,2000);
    if (!Number.isInteger(id)||id<=0) return json({success:false,error:"معرّف المقال غير صالح."},400);
    if (!["draft","pending","published","rejected"].includes(status)) return json({success:false,error:"حالة المقال غير صالحة."},400);

    const article=await db.prepare(`SELECT * FROM articles WHERE id=? LIMIT 1`).bind(id).first();
    ...
    await db.prepare(`
        UPDATE articles SET title_ar=?,title_en=?,content_ar=?,content_en=?,excerpt_ar=?,excerpt_en=?,author_name=?,category=?,image_url=?,status=?,rejection_reason=?,published_at=?,updated_at=CURRENT_TIMESTAMP WHERE id=?
    `).bind(titleAr,titleEn,contentAr,contentEn,excerptAr,excerptEn,authorName,category,imageUrl,status,status==='rejected'?rejectionReason:null,publishedAt,id).run();
...
async function deleteArticle(request,db){
    ...
    if(!result.meta?.changes)return json({success:false,error:"المقال غير موجود."},404);
...
function json(data,status=200){return new Response(JSON.stringify(data),{status,headers:{'Content-Type':'application/json; charset=UTF-8','Cache-Control':'no-store','Access-Control-Allow-Origin':'*','Access-Control-Allow-Headers':'Authorization, Content-Type','Access-Control-Allow-Methods':'GET, POST, PUT, DELETE, OPTIONS'}})}
