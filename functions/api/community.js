export async function onRequest(context) {
    ...
    }catch(error){console.error("Community API error:",error);return json({success:false,error:"حدث خطأ غير متوقع."},500)}
}
...
async function feed(db,url){
    ...
    for(const post of posts){const c=await db.prepare(`SELECT c.id,c.content,c.created_at,c.member_id,COALESCE(pr.display_name,m.full_name) AS author_name,COALESCE(pr.avatar_url,'') AS avatar_url FROM member_comments c JOIN members m ON m.id=c.member_id LEFT JOIN member_profiles pr ON pr.member_id=c.member_id WHERE c.post_id=? AND c.status='published' ORDER BY c.created_at ASC LIMIT 50`).bind(post.id).all();post.comments=c.results||[]}
...
async function profile(db,url,account){
    ...
    member.skills=safeJson(member.skills,[]);member.social_links=safeJson(member.social_links,{});
...
async function authenticatedAccount(request,db){const token=parseCookies(request.headers.get("Cookie")||")["medlife_member_session"];if(!token)return null;return await db.prepare(`SELECT a.*,m.full_name,m.status AS member_status FROM member_sessions s JOIN member_accounts a ON a.id=s.account_id JOIN members m ON m.id=a.member_id WHERE s.id=? AND datetime(s.expires_at)>datetime('now') AND a.account_status='active' AND m.status='active' LIMIT 1`).bind(token).first()}
...
function clean(v,m){return String(v??'').trim().slice(0,m)}function safeJson(v,f){try{return v?JSON.parse(v):f}catch{return f}}function parseCookies(v){const o={};for(const p of v.split(/;\s*/)){const i=p.indexOf('=');if(i>0)o[p.slice(0,i)]=decodeURIComponent(p.slice(i+1))}return o}function json(data,status=200){return new Response(JSON.stringify(data),{status,headers:{'Content-Type':'application/json; charset=UTF-8','Cache-Control':'no-store','Access-Control-Allow-Origin':'*','Access-Control-Allow-Headers':'Content-Type','Access-Control-Allow-Methods':'GET, POST, DELETE, OPTIONS'}})}
