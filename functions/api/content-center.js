...
function mapPage(page) {
  ...
  return {
    id: page.id,
    url: page.url ?? null,
    title: textProperty(p['عنوان المحتوى']),
    cell: textProperty(p['الخلية']),
    unit: textProperty(p['القسم / الوحدة']),
    status: textProperty(p['الحالة']),
    priority: textProperty(p['الأولوية']),
    author: textProperty(p['الكاتب']),
    cellManager: textProperty(p['مسؤول الخلية']),
    due: dateProperty(p['تاريخ التسليم الفعلي']),
    publish: dateProperty(p['تاريخ النشر']),
    updated: page.last_edited_time ?? '',
    published: p['تم النشر']?.checkbox === true,
    designUrl: textProperty(p['رابط التصميم']) || null,
    publishedUrl: textProperty(p['رابط المنشور']) || null,
  };
}
...
async function notionQuery(env, body = {}) {
  ...
  const response = await fetch(`https://api.notion.com/v1/data_sources/${DATA_SOURCE_ID}/query`, {
    ...
    body: JSON.stringify({ page_size: DEFAULT_PAGE_SIZE, ...body }),
  });
  const payload = await response.json().catch(() => ({}));
  if (!response.ok) {
    const message = payload?.message || payload?.code || `Notion API returned ${response.status}`;
...
export async function onRequest(context) {
  ...
  try {
    ...
    body.sorts = [{ property: 'تاريخ آخر تحديث', direction: 'descending' }];

    const payload = await notionQuery(env, body);
    const items = (payload.results ?? []).map(mapPage);
    return json({
      success: true,
      identity: { label: 'Notion Content Hub 2.0' },
    ...
    const message = error instanceof Error ? error.message : 'Unable to load Notion data.';
