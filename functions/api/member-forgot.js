...
export async function onRequest(context) {
    ...
    try {
        ...
        const member = await env.DB.prepare(`
            SELECT a.id AS account_id, a.account_status, m.id AS member_id, m.full_name, m.email, m.status
            FROM members m
        ...
        if (!member || member.account_status !== "active" || !(member.status === "active" || member.status === "approved")) {
            return json({ success: true, message: generic });
        }

        const recent = await env.DB.prepare(`
            SELECT id FROM member_password_reset_tokens
            WHERE account_id = ?
              AND used_at IS NULL
              AND datetime(created_at) > datetime('now', ?)
            LIMIT 1
        `).bind(member.account_id, `-${COOLDOWN_MINUTES} minutes`).first();
        ...
        `).bind(member.account_id).run();

        const token = randomToken();
        const tokenHash = await sha256Hex(token);

        await env.DB.prepare(`
            INSERT INTO member_password_reset_tokens(account_id, token_hash, expires_at)
            VALUES(?, ?, datetime('now', '+${RESET_MINUTES} minutes'))
        `).bind(member.account_id, tokenHash).run();
        ...
        return json({ success: false, error: "تعذر تنفيذ طلب إعادة تعيين كلمة المرور حالياً." }, 500);
    }
}
