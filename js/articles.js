...
document.addEventListener("DOMContentLoaded", () => {
    ...
    // These are the three currently published articles. The first is a static
    // GitHub article; the family-planning article was submitted through the
    // publishing system and its working canonical reader is /article-reader-v3?id=3.
    const staticArticles = [
        ...
        {id:"3",title_ar:"وسائل تنظيم الأسرة: التخطيط الواعي لحياة أسرية متوازنة",title_en:"Family Planning Methods: Informed Planning for a Balanced Family Life",excerpt_ar:"دليل شامل حول وسائل تنظيم الأسرة الهرمونية وغير الهرمونية ومزايا وعيوب كل وسيلة.",author_name:"الممرضة صفا علاء الدين الحكيم",category:"توعية صحية",image_url:"",status:"published",created_at:"2026-08-26T00:00:00",url:"/article-reader-v3?id=3",icon:"fa-people-roof",source:"api"}
    ];

    let articles = [];
    loadArticles();

    function addSubmissionCTA(){const hero=document.querySelector(".hero-content");if(!hero||document.getElementById("submitArticleCTA"))return;const wrapper=document.createElement("div");wrapper.id="submitArticleCTA";wrapper.style.cssText="display:flex;justify-content:center;gap:10px;flex-wrap:wrap;margin-top:24px";wrapper.innerHTML=`<a href="submit-article-v2.html" style="display:inline-flex;align-items:center;gap:9px;padding:12px 20px;border-radius:13px;background:#FF2A54;color:#fff;font-weight:800;text-decoration:none"><i class="fa-solid fa-pen-to-square"></i> أرسل مقالتك للنشر</a>`;hero.appendChild(wrapper)}
    ...
    async function loadArticles(){showLoading();articles=[...staticArticles];populateCategories();renderArticles();try{const response=await fetch("/api/articles",{method:"GET",headers:{Accept:"application/json"},cache:"no-store"});if(!response.ok)throw new Error(`Articles API returned ${response.status}`);const data=await response.json();if(!data.success||!Array.isArray(data.articles))throw new Error(data.error||"Unable to load articles.");const remoteArticles=data.articles.filter(a=>String(a.status||"").toLowerCase()==="published").map(a=>({...a,source:"api",slug:slugify(a.title_ar||a.title_en||a.id)}));const staticIds=new Set(staticArticles.map(a=>String(a.id)));const extraArticles=remoteArticles.filter(a=>!staticIds.has(String(a.id))&&!staticIds.has(String(a.slug||"")));articles=[...staticArticles,...extraArticles];populateCategories();renderArticles()}catch(error){console.warn("MedLife articles API unavailable; showing static articles:",error)}}
    ...
    function createArticleCard(a){const title=escapeHTML(a.title_ar||a.title_en||"بدون عنوان"),excerpt=escapeHTML(a.excerpt_ar||a.excerpt_en||"اضغط لقراءة المقال كاملاً."),category=escapeHTML(a.category||"Medical Knowledge"),author=escapeHTML(a.author_name||"MedLife"),date=formatDate(a.created_at),image=a.image_url?escapeAttribute(a.image_url):"";
        let url;
        if (String(a.id) === "3" || /وسائل تنظيم الأسرة|تنظيم الأسرة/.test(String(a.title_ar||""))) {
            url = "/article-reader-v3?id=3";
        } else if (a.source === "static") {
            url = a.url;
        } else {
            url = `/article-reader-v5.html?slug=${encodeURIComponent(a.slug||slugify(a.title_ar||a.title_en||a.id))}`;
        }
